...
class AdminClient {
    ...
    usersList() {
        return adminClient(this.config).then(client => { return client.users.find(this.config.realm)});
    }
    ...
    resetPasswordEmail(user, redirectUri) {
        return adminClient(this.config)
            .then(client =>  resetPasswordEmail(client, this.config.realm, user, redirectUri))
            .then(() => true)
            .catch(() => false)            
    }
...
async function resetPasswordEmail(client, realm, userId, redirectUri) {
    return client.users.executeActionsEmail(realm, userId, { 
